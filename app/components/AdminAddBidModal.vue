  ...
  >
    <div class="my-8 w-full max-w-lg rounded-2xl bg-white shadow-xl">
      <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
        <h2 class="text-lg font-semibold text-slate-800">Add bid</h2>
        <button
      ...
      <form class="space-y-5 px-6 py-5" @submit.prevent="save">
        ...
        <div>
          ...
          <input v-model="form.bid_date" type="date" :max="maxBidDate" :class="inputClass" />
...
import type { BidJobType, BidStatus } from '~/types/bids'
...
async function save() {
  ...
  try {
    ...
    error.value = e?.data?.error || e?.message || 'Failed to add bid.'
  } finally {
    loading.value = false
  }
}
