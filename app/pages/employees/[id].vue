            ...
            <table class="w-full text-sm">
              ...
              <tbody class="divide-y divide-slate-100">
                ...
                >
                  ...
                  <td class="px-6 py-3 text-slate-600">{{ formatDateTime(m.meeting_at) }}</td>
                  <td class="px-6 py-3 text-slate-600">{{ meetingOutcomeLabel(m.meeting_outcome) }}</td>
                </tr>
                <tr v-if="employee.meetings.length === 0">
                  <td colspan="4" class="px-6 py-10 text-center text-slate-400">No meetings assigned yet.</td>
                </tr>
              </tbody>
            </table>
          </div>
            ...
            <table class="w-full text-sm">
              ...
              <tbody class="divide-y divide-slate-100">
                ...
                >
                  ...
                  <td class="px-6 py-3 text-slate-600">{{ projectStatusLabel(p.status) }}</td>
                  <td class="px-6 py-3 text-slate-600">{{ formatDate(p.start_date) }}</td>
                </tr>
                <tr v-if="employee.projects.length === 0">
                  <td colspan="4" class="px-6 py-10 text-center text-slate-400">No projects assigned yet.</td>
                </tr>
              </tbody>
            </table>
          </div>
