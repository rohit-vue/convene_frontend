<template>
  <div>
        ...
        <table class="w-full min-w-[720px] text-sm">
          ...
          <tbody class="divide-y divide-border">
            <tr
              ...
              @click="openProject(p)"
            >
              <td class="px-4 py-4 font-medium text-fg sm:px-6">{{ p.name }}</td>
              <td class="px-4 py-4 text-fg-muted sm:px-6">{{ p.client_name || '—' }}</td>
              <td class="px-4 py-4 text-fg-muted sm:px-6">{{ p.assignee_name || '—' }}</td>
              ...
              <td class="px-4 py-4 text-fg-muted sm:px-6">{{ formatDate(p.start_date) }}</td>
              <td class="px-4 py-4 text-fg-muted sm:px-6">{{ jobTypeLabel(p.job_type) }}</td>
            </tr>
            <tr v-if="adminFilteredProjects.length === 0">
              <td colspan="7" class="px-4 py-12 text-center text-sm text-fg-subtle sm:px-6">
                {{ adminEmptyMessage }}
              </td>
            </tr>
          </tbody>
    ...
    <template v-else>
      ...
      <template v-else>
      <div v-if="projects && projects.length" class="mb-6 flex flex-nowrap items-center gap-2 overflow-x-auto py-1">
        <button
          v-for="f in filters"
          :key="f.value"
          class="shrink-0 whitespace-nowrap rounded-full px-3 py-1.5 text-xs font-medium transition"
          :class="activeFilter === f.value ? 'border border-transparent bg-indigo-600 text-white shadow-sm' : 'border border-border bg-surface text-fg-muted hover:bg-elevated'"
          @click="activeFilter = f.value"
        >
          {{ f.label }}
          <span :class="activeFilter === f.value ? 'text-white/70' : 'text-fg-subtle'">{{ countFor(f.value) }}</span>
        </button>
      </div>
      ...
      <div v-if="filteredProjects.length" class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        <div
          ...
          :key="p.id"
          class="flex cursor-pointer flex-col rounded-2xl border border-border bg-surface p-6 shadow-sm transition hover:border-indigo-100 hover:shadow-md"
          @click="openProject(p)"
        >
            <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              <h3 class="truncate font-semibold text-fg">{{ p.name }}</h3>
              <p v-if="p.client_name" class="mt-0.5 truncate text-sm text-fg-muted">{{ p.client_name }}</p>
            </div>
          ...
          <p class="mt-3 line-clamp-3 flex-1 text-sm text-fg-muted">
            {{ p.job_description || p.description || 'No details added yet.' }}
          </p>

          <div class="mt-4 flex flex-wrap items-center gap-2 text-xs">
            <span v-if="p.job_type" class="rounded-full bg-elevated px-2.5 py-1 font-medium text-fg-muted">
            ...
            <span v-if="p.due_date" class="inline-flex items-center gap-1 text-fg-muted">
      ...
      <div v-else class="rounded-2xl border border-dashed border-border bg-surface p-12 text-center">
        ...
        <h3 class="mt-4 font-semibold text-fg">{{ activeFilter === 'all' ? 'No projects yet' : 'No projects in this status' }}</h3>
        <p class="mt-1 text-sm text-fg-muted">
...
function assignmentBadgeClass(status) {
  ...
  if (status === 'declined') return 'bg-red-50 text-red-600 dark:bg-red-950 dark:text-red-400'
