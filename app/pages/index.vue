<template>
  <div>
    <div class="mb-6 sm:mb-8">
      ...
      <p class="mt-1 text-sm text-fg-muted">Welcome back, here's what's happening today.</p>
    </div>
    ...
    <template v-else>
      ...
      <div :class="statsGridClass">
        <div
          ...
          :key="card.label"
          class="rounded-2xl border border-border bg-surface p-4 shadow-sm transition hover:shadow-md sm:p-6"
        >
          ...
          <p v-else class="mt-1 text-xs font-medium text-fg-subtle">{{ card.trend }}</p>
        </div>
      </div>
      ...
      <div class="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div class="rounded-2xl border border-border bg-surface p-4 shadow-sm sm:p-6">
          <h2 class="text-lg font-semibold text-fg">Project status</h2>
          <p class="mt-0.5 text-sm text-fg-muted">{{ scopeLabel }} projects by status</p>
          <div class="mt-4 space-y-3">
            <div v-for="item in projectBreakdown" :key="item.key" class="flex items-center gap-2 sm:gap-3">
              <span class="w-16 shrink-0 text-xs text-fg-muted sm:w-24 sm:text-sm">{{ item.label }}</span>
              ...
              <span class="w-8 shrink-0 text-right text-sm font-semibold text-fg">{{ item.count }}</span>
            </div>
          </div>
        </div>

        <div class="rounded-2xl border border-border bg-surface p-4 shadow-sm sm:p-6">
          <h2 class="text-lg font-semibold text-fg">Meeting outcomes</h2>
          <p class="mt-0.5 text-sm text-fg-muted">{{ scopeLabel }} meetings by outcome</p>
          <div class="mt-4 space-y-3">
            <div v-for="item in meetingBreakdown" :key="item.key" class="flex items-center gap-2 sm:gap-3">
              <span class="min-w-0 w-20 shrink-0 text-xs leading-tight text-fg-muted sm:w-32 sm:text-sm">{{ item.label }}</span>
              <div class="h-2 flex-1 overflow-hidden rounded-full bg-elevated">
                <div
                  class="h-full rounded-full transition-all"
                  :class="item.barClass"
        ...
        <div class="rounded-2xl border border-border bg-surface p-4 shadow-sm sm:p-6 lg:col-span-2">
          ...
          <ul v-if="overview?.activity?.length" class="mt-4 space-y-4">
            <li v-for="(a, i) in overview.activity" :key="`${a.at}-${i}`" class="flex items-start gap-3">
              ...
              <div>
                <p class="text-sm text-fg">{{ a.text }}</p>
                <p class="text-xs text-fg-subtle">{{ formatActivityTime(a.at) }}</p>
              </div>
            </li>
          </ul>
          <p v-else class="mt-4 text-sm text-fg-subtle">No recent activity yet.</p>
        </div>
        ...
        <div class="rounded-2xl border border-border bg-surface p-4 shadow-sm sm:p-6">
          ...
          <div v-if="isEmployee" class="mt-4 space-y-3">
            ...
            >
              ...
              <p class="mt-1 text-sm font-medium text-fg">{{ m.project_name }}</p>
              <p class="mt-0.5 text-xs text-fg-muted">{{ m.client_name }} · {{ formatMeetingTime(m.meeting_at) }}</p>
              <button
            ...
            >
              ...
              <p class="mt-1 text-sm font-medium text-fg">{{ p.name }}</p>
            ...
            <p v-if="!pendingMeetings.length && !pendingProjects.length" class="text-sm text-fg-subtle">
          ...
          <div v-else class="mt-4 space-y-3">
            <NuxtLink
              to="/meetings"
              class="flex items-center justify-between rounded-xl bg-elevated px-4 py-3 text-sm font-medium text-fg transition hover:bg-elevated"
            >
            ...
            <NuxtLink
              ...
              class="flex items-center justify-between rounded-xl bg-elevated px-4 py-3 text-sm font-medium text-fg transition hover:bg-elevated"
            >
              View all daily logs <span>&rarr;</span>
            </NuxtLink>
            <NuxtLink
              to="/employees"
              class="flex items-center justify-between rounded-xl bg-elevated px-4 py-3 text-sm font-medium text-fg transition hover:bg-elevated"
...
const primaryCards = computed(() => {
  const cards = [
    {
      ...
      iconBg: 'bg-indigo-50 text-indigo-600 dark:bg-indigo-900/40 dark:text-indigo-300',
  ...
  cards.push({
    ...
    iconBg: 'bg-amber-50 text-amber-600 dark:bg-amber-900/40 dark:text-amber-300',
